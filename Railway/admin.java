...
class admin
{
	...
	void disp_chart() throws Exception
	{
		try
		{
			...
			System.out.println("***************************************************************************************************************************************************************************************");
			Statement st=c.createStatement();
			ResultSet r=st.executeQuery("select * from chart where tnum='"+tn+"' ");
			while(r.next())
			{
				setwR(r.getLong(2),r.getString(3),r.getInt(4),r.getString(5),r.getInt(6),r.getString(7),r.getString(8),r.getTimestamp(9),r.getDate(10),20);
			}
		}
		catch(Exception e)
		{
